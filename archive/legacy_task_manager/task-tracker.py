...
# Path to the project plan - use the consolidated project plan
PROJECT_PLAN_PATH = os.path.join(PROJECT_ROOT, "docs/planning/consolidated-project-plan.md")
...
CONTEXT_CACHE_PATH = os.path.join(PROJECT_ROOT, "docs/context-cache.json")
# Path to the task prompt templates
PROMPT_TEMPLATES_PATH = os.path.join(PROJECT_ROOT, "docs/templates/task-prompt-templates.json")
...
def load_task_tracking() -> Dict[str, Any]:
    ...
    if os.path.exists(TASK_TRACKING_PATH):
        ...
        return {
            "current_phase": "Integration Setup",
            "current_task": "API Contract Definition",
...
def extract_tasks_from_plan(plan_text: str) -> Dict[str, Any]:
    ...
    # Regular expression to match phase headers (### Phase X: Phase Name)
    phase_pattern = re.compile(r'###\s+Phase\s+\d+:\s+(.*?)\s*$')
    
    # Regular expression to match task items in the consolidated project plan format
    # **Task X.X.X**: Create a comprehensive component map with relationships
    task_pattern = re.compile(r'\*\*Task\s+\d+\.\d+\.\d+\*\*:\s+(.*?)$')
    
    # Alternative task pattern for other formats
    alt_task_pattern = re.compile(r'-\s+\*\*(.*?)\*\*:')
    ...
    for line in lines:
        ...
        if current_phase:
            ...
            alt_match = alt_task_pattern.search(line)
            
            if task_match:  # Main task with Task X.X.X format
            ...
            elif alt_match and not task_match:  # Alternative task format
                task_name = alt_match.group(1)
                if "Task" in task_name:
                    task_name = task_name.replace("Task", "").strip()
                # Default status is Planned
                status = "📅 Planned"
                status_match = status_pattern.search(line)
                if status_match:
            ...
            # Check if this line is a sub-task item
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Task Tracker for AI-Orchestration-Platform")
    ...
    parser.add_argument('--template', type=str, default="default", help="Template to use for task prompt (default, concise, detailed, integration-task, etc.)")
    parser.add_argument('--sub-task', type=str, help="Specify a sub-task name when completing a task")
    parser.add_argument('--json', action='store_true', help="Output in JSON format (for status)")
    ...
    try:
        ...
        print("Please run the initialization script first: ./scripts/init-task-system.sh")
        return
    ...
    if args.complete:
        ...
        if args.json:
            ...
            if tracking_data.get("sub_task"):
                status_data["sub_task"] = tracking_data["sub_task"]
            
            status_data["completed_tasks_count"] = len(tracking_data["completed_tasks"])
